...
import { type JobPosting, JobPostingSchema } from "@/types/jobPosting";
 ...
 *
 * Downstream agents (resumeScorer, resumeWriter) depend on this output,
 * so the structured output mode is enforced — the model MUST return valid
 * JSON matching the schema or Mastra will throw a validation error.
 */
export const jobParserAgent = new Agent<"job-parser-agent", ToolsInput, JobPosting>({
  id: "job-parser-agent",
...
hourly rates to annual (multiply by 2080) and note that in currency as normal.
...
- Return only the structured JSON. No preamble, explanation, or commentary.
  ...
  // vNext default execution options used by generate()/stream() and exposed via getDefaultOptions().
  defaultOptions: {
    structuredOutput: {
